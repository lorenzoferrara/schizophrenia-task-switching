...
## Inputs: subject event files from `2 - materiale/events recording`.
## Output: switch/no-switch summaries and switch-cost comparisons.
project_paths_candidates <- c(
  "../project_paths.R",
  "project_paths.R",
  "2 - materiale/script/project_paths.R"
)
project_paths_file <- project_paths_candidates[file.exists(project_paths_candidates)][1]
if (is.na(project_paths_file)) {
  stop("Cannot locate project_paths.R. Run from repo root or script folder.")
}
source(project_paths_file)
setwd(project_path("2 - materiale", "events recording"))
...

{
  ...
  p1 <- ggplot(data.frame(g = group, Reaction.Time = times), aes(x = g, y = Reaction.Time)) +
    ...
    theme(text = element_text(size = 20), legend.position = "", plot.title = element_text(hjust = 0.5, size = 20, face = "bold")) +
    xlab("") +
    ylab("") +
...

{
  ...
  pp <- ggplot(data.frame(Diagnosis = c(rep("Control", 125), rep("Schz", 50)), Switch.Cost = c(switchcost_control, switchcost_schz)), aes(x = Diagnosis, y = Switch.Cost)) +
    ...
    theme_minimal() +
    theme(text = element_text(size = 20), legend.position = "", plot.title = element_text(hjust = 0.5, size = 17, face = "bold")) +
    xlab("") +
    ylab("") +
    labs(title = "Switch costs")

...
# prima PC
disegno_load<-directions[,1]-medie
barplot(disegno_load)

M_loads<-mean(disegno_load)
SD_loads<-sd(disegno_load)
alti_pc1 = which( disegno_load > M_loads+2*SD_loads | disegno_load < M_loads-2*SD_loads)

{
  colors = rep('grey', 83)
  colors[alti_pc1]='red'
  x11()
  barplot(disegno_load, ylim=c(-0.10,0.10), col=colors)
...
disegno_load<-directions[,2]
barplot(disegno_load)

M_loads<-mean(disegno_load)
SD_loads<-sd(disegno_load)
alti_pc1 = which( disegno_load > M_loads+1.75*SD_loads | disegno_load < M_loads-1.75*SD_loads)

{
  colors = rep('grey', 83)
  colors[alti_pc1]='red'
  x11()
  barplot(disegno_load, ylim=c(-0.3,0.3), col=colors)
  dev.copy(jpeg, 'loadings_pc1.jpeg')
}
...
plot(1:175,zmap.pc$scores[,2], main='scores 2nd PC', xlab='subjects', ylab='score')
points(1:125,zmap.pc$scores[1:125,2],col='red')
abline(h=mean(zmap.pc$scores[1:125,2]))

## terza

disegno_load<-directions[,3]
barplot(disegno_load)

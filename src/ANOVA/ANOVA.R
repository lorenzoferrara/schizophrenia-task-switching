...
setwd(project_path("data", "raw", "events-recording"))

# import recordings file names and participants.csv
BigList <- list.files(pattern = "*.tsv")
BigList

participants <- read.table("../project-data/participants.csv", header = T)
p <- read.table("../project-data/participants.csv", header = T)
...
for (individui in BigList) {
  ii <- ii + 1
  da_leggere <- read.table(individui, header = T) # import sub-*****_task-taskswitch_events.tsv
...
color <- c("lightskyblue1", "tomato3")
{
  ...
  boxplot(Congruent_switch_Sani, Congruent_switch_Schz, Incongruent_switch_Sani, Incongruent_switch_Schz,
    ...
    ), cex.axis = 0.9, ylab = "Mean Reaction Time"
  )
...
dati <- c(
  Congruent_switch_Sani, Congruent_switch_Schz, Incongruent_switch_Sani, Incongruent_switch_Schz, Congruent_Noswitch_Sani,
...
{
  x11(width = 9)
  ggplot(dataframe, aes(x = trattamento, y = dati, fill = Diagnosis)) +
    geom_boxplot() +
    scale_fill_manual(values = c("royalblue", "orange2")) +
    theme(
      panel.background = element_rect(fill = "white"), text = element_text(size = 16, face = "bold"), legend.position = "",
...
rispost <- rispost[rispost > 0.4]
{
  ...
  pp <- ggplot(data.frame(Diagnosis = c(rep("Control", 123), rep("Schz", 48)), MediaRisposte = rispost), aes(x = Diagnosis, y = MediaRisposte)) +
    ...
    ylim(0, 1) +
    theme_minimal() +
    theme(text = element_text(size = 18), legend.position = "", plot.title = element_text(hjust = 0.5, size = 18, face = "bold")) +
    xlab("") +
    ylab("") +
    labs(title = "General accuracy")
  pp
...

{
  ...
  pp <- ggplot(data.frame(Diagnosis = c(rep("Control", 125), rep("Schz", 50)), MediaRisposte = errori$performance1_on_switch), aes(x = Diagnosis, y = MediaRisposte)) +
    ...
    ylab("") +
    theme_minimal() +
    theme(legend.position = "", plot.title = element_text(hjust = 0.5, , size = 17, face = "bold"), text = element_text(size = 20)) +

...
part_list <- read.delim(project_path("data", "raw", "project-data", "participants.csv"))
barratt_raw_data <- read.csv(project_path("src", "barratt", "barratt_raw_data.txt"), sep = "")
...

{
  ...
  pp <- ggplot(data.frame(Diagnosis = part_list$diagnosis, Barratt.Score = score), aes(x = Diagnosis, y = Barratt.Score)) +
    ...
    theme_minimal() +
    theme(legend.position = "", text = element_text(size = 20), plot.title = element_text(hjust = 0.5, size = 20, face = "bold")) +
    xlab("") +
    ylab("") +
    labs(title = "Barratt Scores")
...

{
  ...
  pp <- ggplot(data = data.frame(Barratt.Score = barratt_raw_data$total_score, Reaction.Time = times, diag = part_list$diagnosis), aes(Barratt.Score, Reaction.Time)) +
    ...
    theme(
      legend.position = " ", axis.title = element_text(size = 14, face = "bold"), legend.title = element_blank(),

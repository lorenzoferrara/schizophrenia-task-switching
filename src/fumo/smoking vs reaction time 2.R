...
load(project_path("data", "processed", "workspaces", "smoking.RData"))
...

{
  ...
  barplot(tapply(y, x, mean),
    names.arg = levels(x), ylim = c(0, max(y)),
...
for (i in 1:(g - 1)) {
  for (j in (i + 1):g) {
    ...
    ICrange <- rbind(ICrange, as.numeric(c(
      Mediag[i] - Mediag[j] - qt(1 - alpha / (2 * k), n - g) * sqrt(S * (1 / ng[i] + 1 / ng[j])),
      Mediag[i] - Mediag[j] + qt(1 - alpha / (2 * k), n - g) * sqrt(S * (1 / ng[i] + 1 / ng[j]))
    )))
  }
...

{
  ...
  df <- data.frame(
    differences = c("cur-ex", "cur-not", "ex-not"),

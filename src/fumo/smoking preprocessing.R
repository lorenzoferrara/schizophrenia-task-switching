...
phenotype.demographics <- read.delim(project_path("data", "raw", "covariates", "phenotype.demographics.tsv"), sep = " ")
...
for (ii in 1:length(cols$id))
{
  if (scaled_t[ii] <= -0.5) {
    cols$index[ii] <- 0
  } else if (scaled_t[ii] > -0.5 && scaled_t[ii] < 0.5) {
    cols$index[ii] <- 1
  } else {
    cols$index[ii] <- 2
  }
...
save(list = c("ex_smokers", "current_smokers", "not_smokers"), file = project_path("data", "processed", "workspaces", "smoking.RData"))

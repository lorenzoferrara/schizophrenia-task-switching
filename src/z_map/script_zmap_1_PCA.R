...
save(list = c("con_reg"), file = project_path("data", "processed", "workspaces", "connectivity.map.RData"))
...
jpeg(filename = file.path(output_dir, "Screeplot.jpeg"))
...
for (i in 1:10) {
  jpeg(file.path(output_dir, names[i]), width = 350, height = 350)

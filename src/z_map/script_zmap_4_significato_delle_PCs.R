...

{
  ...
  p2 <- ggplot(data.frame(regione = dir1), aes(x = names(dir1), y = regione)) +
    geom_bar(stat = "identity", colour = colors11, fill = colors1, ) +
    xlab("1st PC") +
    ylab("") +
...

{
  ...
  p2 <- ggplot(data.frame(regione = dir2), aes(x = names(dir2), y = regione)) +
    geom_bar(stat = "identity", colour = colors22, fill = colors2, ) +
    xlab("2nd PC") +
    ylab("") +
    theme_minimal(base_size = 15) +
    theme(legend.position = "", axis.text.x = element_blank()) +
...

{
  ...
  p2 <- ggplot(data.frame(regione = dir3), aes(x = names(dir3), y = regione)) +
    geom_bar(stat = "identity", colour = colors33, fill = colors3, ) +
    xlab("") +
    ylab("") +
    theme_minimal(base_size = 15) +
    theme(legend.position = "", axis.text.x = element_blank(), plot.title = element_text(hjust = 0.5)) +
    ylim(minimo, massimo) +
    labs(title = "1st PC")
  p2
}
...
output_dir <- project_path("results", "figures", "boxplot-regioni")
...
for (i in 0:5) {
  ...
  dev.copy(jpeg, file.path(output_dir, paste("plot", i, ".jpeg", sep = "")))
}
...
library(ggplot2)
{
  ...
  pp <- ggplot(data.frame(Diagnosis = c(rep("Control", 125), rep("Schz", 50)), Scores = scores[, 1]), aes(x = Diagnosis, y = Scores)) +
    ...
    scale_fill_manual(values = c("royalblue", "orange2")) +
    theme_minimal() +
    theme(legend.position = "", text = element_text(size = 16, face = c("bold")), plot.title = element_text(hjust = 0.5, size = 18, face = "bold")) +
    xlab("") +
    ylab("") +
    labs(title = "Scores on 1st PC")
  pp
}

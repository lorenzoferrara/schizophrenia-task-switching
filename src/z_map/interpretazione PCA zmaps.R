...
for (i in high_pc) {
  for (j in 1:43) {
    if (is.na(names_regions[j, 1])) {
      names_regions[j, 1] <- 0
    }
    if (is.na(names_regions[j, 2])) {
      names_regions[j, 2] <- 0
    }
...
elenco_copia <- read.delim(file.path(events_dir, "elenco_copia.txt"), header = T)
elenco <- read.delim(file.path(events_dir, "elenco.txt"), header = T)
p <- read.delim(project_path("data", "raw", "project-data", "participants.csv"))
...
for (i in 1:175) {
  val <- elenco$v[i]
  s <- read.delim(file.path(events_dir, val))
...
jpeg(filename = project_path("results", "figures", "Screeplot.jpeg"))
...
output_dir <- project_path("results", "figures", "pca-loadings")
...
for (i in 1:10)
{
  jpeg(file.path(output_dir, nomi[i]), width = 350, height = 350)

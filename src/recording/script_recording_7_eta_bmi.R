...
load(project_path("data", "processed", "workspaces", "times.RData"))

elenco_copia <- read.delim(project_path("data", "raw", "events-recording", "elenco_copia.txt"), header = T)
elenco <- read.delim(project_path("data", "raw", "events-recording", "elenco.txt"), header = T)
...

{
  ...
  pp <- ggplot(data = data.frame(Age = p$age, Reaction.Time = times, diag = p$diagnosis), aes(Age, Reaction.Time)) +
    ...
    theme(
      axis.title = element_text(size = 14, face = "bold"), legend.position = "",
...
phenotype.health <- read.delim(project_path("data", "raw", "covariates", "phenotype health.tsv"), header = T, sep = " ")
